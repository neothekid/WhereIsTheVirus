...
extension InteractiveTransitionAnimation {
    ...
    fileprivate static let push = InteractiveTransitionAnimation(duration: duration) { context in
        ...
        guard let toView = context.view(forKey: .to) ?? context.viewController(forKey: .to)?.view,
            let fromView = context.view(forKey: .from) ?? context.viewController(forKey: .from)?.view else {
                return
        }
